...
#			channel_mask, channel_types, channel_positions = \
#                                      self.GenerateChannelMask( data['_slot'],data['_channel'])
#                         
...
#                        # This gets done a second time because we've removed the "Off" channels.
#			channel_mask, channel_types, channel_positions = \
#                                      self.GenerateChannelMask( data_series['_slot'],data_series['_channel'])
...
#                                                           'ChannelTypes','ChannelPositions'])
#				print('Written to {} at {:4.4} seconds'.format(output_filename,\
...
#			df.to_hdf(output_filename,key='raw')
#			end_time = time.time()
#			print('{} events written in {:4.4} seconds.'.format(global_evt_counter,\
#                                                                            end_time-start_time))
#		else:
#			return df
	...
	####################################################################
        def ReadFile(filename, max_num_of_spills=-1):
            start_time = time.time()
                ...
                while True:
                    ...
                    if first_word_of_spillhdr == '0xabbaabba':
                        spill_dict, words_read, last_word_read = \
                            ReadSpill(infile, first_word_of_spillhdr)
	...
	####################################################################
        def ReadSpill( infile, first_entry_of_spillhdr ):
            debug = False
            ...
            while True:
                ...
                channel_dict, words_read = ReadChannel(infile)
	...
	####################################################################
        def ReadChannel(infile):
             ...
             while total_words_read < data_buffer_size:
                 ...
                 words_read, event = ReadEvent(infile)
                 total_words_read += words_read
                 events.append(event)
                 num_loops += 1
